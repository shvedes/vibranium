...
printf "\n%s[VIBRANIUM | THEMES]%s Installing icon theme" "${YELLOW}" "${RESET}"
cd ~/.cache
git clone -q https://github.com/PapirusDevelopmentTeam/papirus-icon-theme
cd papirus-icon-theme
mkdir -p "$HOME/.local/share/icons"
cp -r Papirus* "$HOME"/.local/share/icons
printf "\n%s[VIBRANIUM | THEMES]%s Icon theme installed%s" "${YELLOW}" "${GREEN}" "${RESET}"

printf "\n%s[VIBRANIUM | THEMES]%s Installing icon theme patcher" "${YELLOW}" "${RESET}"
...
printf "\n%s[VIBRANIUM | THEMES]%s Icon theme patcher installed%s" "${YELLOW}" "${GREEN}" "${RESET}"
