...
# nameref writes inside it actually update the caller's variable. We redirect
# its output to a temp file, then replay it — no pipe around the function call.
log_case() {
...
# _allowed_list <var_name>
# Converts pipe-delimited OPTION_ALLOWEDS storage (|val1|val2|...|) into a
# plain space-separated string suitable for display and array splitting.
_allowed_list() {
    local raw="${OPTION_ALLOWEDS[$1]-}"
    raw="${raw//|/ }"          # replace every | with a space
    printf '%s' "${raw# }"     # strip the single leading space and print
}
...
# Trigger lazy-load once up front so cache arrays are populated for the
# spot-checks and for reading defaults/ranges/allowed lists below.
helpers::check VIBRANIUM_GLOBAL_USE_OSD > /dev/null 2>&1

section "helpers::check (cache load)"
printf "  Cache sizes:\n"
...
_spot_check() {
    ...
    [[ -n $exp_range   ]] && printf "  %53s range=[%s]\n" "" "${OPTION_MINS[$var]-?}..${OPTION_MAXS[$var]-?}"
    [[ -n $exp_allowed ]] && printf "  %53s allowed='%s'\n" "" "$(_allowed_list "$var")"
}

# OPTION_ALLOWEDS is pipe-delimited: |google|duckduckgo|bing|brave|
...
# OPTION_ALLOWEDS is pipe-delimited (|val1|val2|...|) — convert to array for iteration.
# Replace every | with a space, then let read -ra split on whitespace.
_raw="${OPTION_ALLOWEDS[$SE]-}"; _raw="${_raw//|/ }"
...
_raw="${OPTION_ALLOWEDS[$SE2]-}"; _raw="${_raw//|/ }"
