    ...
    switch $argv[$i]
      ...
      case -i --interval
        ...
        if test $i -gt (count $argv)
          echo "memwatch: --interval requires a value" >&2; return 1
        end
        set interval $argv[$i]
        if not string match -qr '^[0-9]+(\.[0-9]+)?$' -- $interval
          echo "memwatch: interval must be a positive number" >&2; return 1
        end

      case -n --entries
        set i (math $i + 1)
        if test $i -gt (count $argv)
          echo "memwatch: --entries requires a value" >&2; return 1
        end
        set entries $argv[$i]
        if not string match -qr '^[0-9]+$' -- $entries
          echo "memwatch: entries must be a positive integer" >&2; return 1
        end

      case --
        break
      case '-*'
        echo "memwatch: unknown option $argv[$i]" >&2; return 1
    end
    set i (math $i + 1)
  end
