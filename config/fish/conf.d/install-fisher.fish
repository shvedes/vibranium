...
set -l first_run_marker $__fish_config_dir/first-run
...
if test -f $first_run_marker
    ...
    if not test -f $__fish_config_dir/functions/fisher.fish
        echo (set_color yellow)"Warning:"(set_color normal)" missing fish plugin manager! Try "(set_color green)"fix-shell"(set_color normal)
        echo
    end
end
